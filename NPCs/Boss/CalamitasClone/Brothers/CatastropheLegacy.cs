...
{
    [AutoloadBossHead]
    public class CatastropheLegacy : ModNPC
    {
        #region 杂项初始化
        public enum LegacyCatastropheLegacyAttackType
        {
            fireProj,
            charge,
        }

        public static LegacyCatastropheLegacyAttackType[] AttackCycle =>
            [
            LegacyCatastropheLegacyAttackType.charge,// 这是用来标记的，因为调用的时候会+1，取不到第一个，得取一遍回来才能取到
            LegacyCatastropheLegacyAttackType.fireProj,
            ];
        #endregion
        #region SSD
        ...
        public override void SetStaticDefaults()
        {
            ...
            if (!Main.dedServ)
            {
                GlowMask = Request<Texture2D>($"{Gen}/CatastropheLegacy_Glow", AssetRequestMode.AsyncLoad);
            }
        }
        #endregion
        #region SD
        ...
        public override void AI()
        {
            ...
            // 获取NPC实例
            CIGlobalNPC.LegacyCatastropheClone = NPC.whoAmI;
            ...
            switch ((LegacyCatastropheLegacyAttackType)attackType)
            {
                case LegacyCatastropheLegacyAttackType.fireProj:
                    DoBehavior_FireCataclysmFirer(target, attackTimer);
                    break;
                case LegacyCatastropheLegacyAttackType.charge:
        ...
        public void DoBehavior_FireCataclysmFirer(Player target, float attacktimer)
        {
            ...
            if (attacktimer % fireDelay == 0)
            {
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    int projType = ProjectileType<CatastropheBall>();
                    // 偏移向量
                    Vector2 offset = new Vector2(0, 50).RotatedBy(NPC.rotation);
                    Vector2 projectileVelocity = direction * 18f;
                    Vector2 projectileSpawn = NPC.Center + offset;
                    projectileVelocity = projectileVelocity.RotatedBy(MathHelper.PiOver2);

                    Projectile.NewProjectile(NPC.GetSource_FromAI(), projectileSpawn, projectileVelocity, projType, ballDamage, 0f, Main.myPlayer, 0f, 0f);
                }
            }
        ...
        public void SelectNextAttack(int? Skip = null)
        {
            ...
            LegacyCatastropheLegacyAttackType[] attackCycle = AttackCycle;
            if (Skip == null)
            {
                // 递增索引
                currentIndex++;
        ...
        {
            npcLoot.Add(ItemType<BrimstoneFlameblaster>(), 4);
        }
        #endregion
    }
}
