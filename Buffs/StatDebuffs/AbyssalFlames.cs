...
namespace CalamityInheritance.Buffs.StatDebuffs
{
    public class AbyssalFlames : ModBuff
    {
        ...
        internal static void DrawEffects(PlayerDrawSet drawInfo, bool hasDebuffResistance = false)
        {
            ...
            {
                Dust dust = Dust.NewDustPerfect(Player.CIMod().RandomDebuffVisualSpot, Main.rand.NextBool(3) ? 114 : ModContent.DustType<BrimstoneFlame>(), new Vector2(0, Main.rand.NextFloat(-3f, -5f)) + Player.velocity, 0, default, hasDebuffResistance ? 1.1f : 1.6f);
                dust.noGravity = true;
                for (int i = 0; i < 3; i++)
                {
