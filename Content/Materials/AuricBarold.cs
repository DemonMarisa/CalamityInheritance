...
namespace CalamityInheritance.Content.Materials
{
    public class AuricBarold : CIMaterials
    {
        ...
        public override bool PreDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
        {
            ...
            Main.spriteBatch.Draw(texture, position, frame, Color.White, 0, origin, scale, 0, 0);
            Main.spriteBatch.Draw(GlowTexture.Value, position, frame, Color.White, 0, origin, scale, 0, 0);
            return false;
        }
        ...
        {
            var frame = Main.itemAnimations[Item.type].GetFrame(GlowTexture.Value);
            var position = Item.Center - Main.screenPosition;
            var origin = frame.Size() / 2f;
            Texture2D texture = TextureAssets.Item[Type].Value;
            Main.spriteBatch.Draw(texture, position, frame, Color.White, 0, origin, scale, 0, 0);
            Main.spriteBatch.Draw(GlowTexture.Value, position, frame, Color.White, rotation, origin, scale, 0, 0);
