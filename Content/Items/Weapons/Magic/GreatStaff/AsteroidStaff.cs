...
{
    public class AsteroidStaff : CIMagic
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            float meteorSpawnXPos = Main.mouseX + Main.screenPosition.X - realPlayerPos.X;
            float meteorSpawnYPos = Main.mouseY + Main.screenPosition.Y - realPlayerPos.Y;
            if (player.gravDir == -1f)
            {
                meteorSpawnYPos = Main.screenPosition.Y + Main.screenHeight - Main.mouseY - realPlayerPos.Y;
            }
            ...
            {
                meteorSpawnXPos = player.direction;
                meteorSpawnYPos = 0f;
            ...
            {
                realPlayerPos = new Vector2(player.Center.X + (float)(Main.rand.Next(201) * -(float)player.direction) + (Main.mouseX + Main.screenPosition.X - player.position.X), player.MountedCenter.Y - 600f);
                realPlayerPos.X = (realPlayerPos.X + player.Center.X) / 2f + Main.rand.Next(-200, 201);
                realPlayerPos.Y -= 100 * i;
                meteorSpawnXPos = Main.mouseX + Main.screenPosition.X - realPlayerPos.X + Main.rand.Next(-40, 41) * 0.03f;
                meteorSpawnYPos = Main.mouseY + Main.screenPosition.Y - realPlayerPos.Y;
                ...
                float meteorSpawnYOffset = meteorSpawnYPos + Main.rand.Next(-40, 41) * 0.02f;
