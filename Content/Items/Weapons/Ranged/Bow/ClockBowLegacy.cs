...
{
    public class ClockBowLegacy : CIRanged
    {
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            {
                vector2 = new Vector2(player.position.X + player.width * 0.5f + (float)(Main.rand.Next(201) * -(float)player.direction) + (Main.mouseX + Main.screenPosition.X - player.position.X), player.MountedCenter.Y);
                vector2.X = (vector2.X + player.Center.X) / 2f + Main.rand.Next(-200, 201);
                vector2.Y -= 100 * num131;
                ...
                num78 *= num80;
                num79 *= num80;
                float speedX4 = num78 + Main.rand.Next(-600, 601) * 0.02f;
                float speedY5 = num79 + Main.rand.Next(-600, 601) * 0.02f;
            ...
            Vector2 realPlayerPos2 = player.Center - vector2;
