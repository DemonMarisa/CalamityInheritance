...
namespace CalamityInheritance.Content.Items.Weapons.Ranged.Guns
{
    public class HellbornLegacy : CIRanged
    {
        ...
        public override float UseTimeMultiplier(Player player) => 1f - 0.5f * (HellbornBoost * (1f / 600f));

        public override void ModifyWeaponDamage(Player player, ref StatModifier damage) => damage *= 1f + HellbornBoost * (1f / 600f);

        public override void ModifyWeaponKnockback(Player player, ref StatModifier knockback) => knockback *= 1f + (HellbornBoost * (1f / 600f));
        ...
        {
            //在UpdateInventory处理这个计时器可以走自动暂停
            if (HellbornBoost > 0)
                HellbornBoost--;
        }
