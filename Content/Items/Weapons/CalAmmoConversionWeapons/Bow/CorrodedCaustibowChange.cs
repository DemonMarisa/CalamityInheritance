...
namespace CalamityInheritance.Content.Items.Weapons.CalAmmoConversionWeapons.Bow
{
    public class CorrodedCaustibowChange : GlobalItem
    {
        ...
        {
            return item.type == ItemType<CorrodedCaustibow>();
        }
        public override bool Shoot(Item item, Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (CIConfig.Instance.AmmoConversion == true)
            {
                Projectile.NewProjectile(source, position.X, position.Y, velocity.X, velocity.Y, ProjectileType<CorrodedShell>(), damage, knockback, player.whoAmI);
                return false;
            }
            else
                return true;
