...
{
    public class VirulentBeeWave : CIMeleeProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[1] < 60f)
            {
                if (Projectile.ai[0] > 7f)
                {
                    ...
                    {
                        int spawnX = Projectile.width / 2;
                        int spawnY = Projectile.width / 2;
                        int bee = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X + Main.rand.Next(-spawnX, spawnX), Projectile.Center.Y + Main.rand.Next(-spawnY, spawnY), Projectile.velocity.X * 0.5f, Projectile.velocity.Y * 0.5f,
                            ProjectileType<PlagueBeeLegacy>(), player.beeDamage(Projectile.damage / 3), player.beeKB(0f), Projectile.owner);
