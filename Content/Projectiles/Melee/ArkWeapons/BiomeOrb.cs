...
{
    public class BiomeOrb : CIMeleeProj
    {
        ...
        public override void AI()
        {
            ...
            if (astral)
            {
                dustType = DustType<CIAstralOrange>();
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            if (astral)
            {
                target.AddBuff(BuffType<CIAstralInfection>(), debuffTime);
            }
            else if (jungle)
            {
            ...
            {
                target.AddBuff(BuffType<CICrushDepth>(), debuffTime);
            }
            else if (dungeon)
            {
                target.AddBuff(BuffID.Frostburn, debuffTime);
            ...
            {
                target.AddBuff(BuffType<CIHolyFlames>(), debuffTime);
            }
            else if (glow)
            {
                target.AddBuff(BuffType<CISadness>(), debuffTime / 3);
            }
            else if (hell)
            {
                target.AddBuff(BuffType<CIBrimstoneFlames>(), debuffTime);
            }
            else if (holy)
            {
                target.AddBuff(BuffType<CIHolyFlames>(), debuffTime);
            ...
            {
                target.AddBuff(BuffType<CIArmorCrunch>(), debuffTime);
            }
        }
    }
}
