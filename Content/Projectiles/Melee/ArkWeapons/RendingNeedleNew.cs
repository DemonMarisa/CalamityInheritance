...
{
    public class RendingNeedleNew : CIMeleeProj
    {
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            Vector2 start = -Projectile.velocity.SafeNormalize(Vector2.Zero) * 16f;

            return Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), Projectile.Center + start, Projectile.Center + start + Projectile.velocity.SafeNormalize(Vector2.Zero) * bladeLength, 24, ref collisionPoint);
        }
        ...
        public override void AI()
        {
            ...
            float Scale = 0.25f;
            Vector2 pos = Projectile.Center + Projectile.velocity.SafeNormalize(Vector2.Zero) * 30.5f;
            for (int i = 0; i < 20; i++)
            {
            ...
            {
                HeavySmokeParticle smoke = new HeavySmokeParticle(Projectile.Center, Projectile.velocity * 0.5f, Color.Lerp(Color.DodgerBlue, Color.MediumVioletRed, (float)Math.Sin(Main.GlobalTimeWrappedHourly * 6f)), 30, Main.rand.NextFloat(0.6f, 1.2f) * Projectile.scale * 0.6f, 0.28f, 0, false, 0, true);
                smoke.Spawn();

                if (Main.rand.NextBool(3))
                {
                    HeavySmokeParticle smokeGlow = new HeavySmokeParticle(Projectile.Center, Projectile.velocity * 0.5f, Main.hslToRgb(Main.rand.NextFloat(), 1, 0.7f), 20, Main.rand.NextFloat(0.4f, 0.7f) * Projectile.scale * 0.6f, 0.8f, 0, true, 0.05f, true);
                    smokeGlow.Spawn();
                }
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 sparkCenter = Projectile.Center - Projectile.velocity.SafeNormalize(Vector2.Zero) * 30.5f - Main.screenPosition;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                SquishyLightParticle energyLeak = new SquishyLightParticle(Projectile.Center + Projectile.velocity.SafeNormalize(Vector2.Zero) * 40f, particleSpeed, Main.rand.NextFloat(0.3f, 0.6f), Color.Cyan, 60, 1, 1.5f, hueShift: 0.02f);
                energyLeak.Spawn();
            }
        }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                SquishyLightParticle energyLeak = new SquishyLightParticle(particleOrigin, particleSpeed, Main.rand.NextFloat(0.3f, 0.6f), Color.Cyan, 60, 1, 1.5f, hueShift: 0.02f);
                energyLeak.Spawn();
            }
        }
    }
