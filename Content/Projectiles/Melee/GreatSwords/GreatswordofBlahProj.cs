...
{
    public class GreatswordofBlahProj : CIMeleeProj
    {
        ...
        public override void AI()
        {
            ...
            if (whiteLightTimer == 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    for (i = 0; i < 1; i++)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(Math.Sin(offsetAngle) * 0.6f), (float)(Math.Cos(offsetAngle) * 0.6f), ProjectileType<GreatswordofBlahWhiteOrb>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, (float)(-Math.Sin(offsetAngle) * 0.6f), (float)(-Math.Cos(offsetAngle) * 0.6f), ProjectileType<GreatswordofBlahWhiteOrb>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                    }
                }
                whiteLightTimer = 5;
            ...
            Projectile.HomeInNPC(200f, 10f, 20f);
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y - 100, 0f, 0f, ProjectileType<GreatswordofBlahAura>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0f);
            }
        }
    }
}