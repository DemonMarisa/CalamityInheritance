...
{
    public class EntropicFlechetteLegacy : CIMeleeProj
    {
        public override string Texture => CIProjectiles_Melee.EntropicFlechette1.Path;
        ...
        public override void AI()
        {
            ...
            else
            {
                for (int i = 0; i < 2; i++)
                {
                    Vector2 dustRotate = -Vector2.UnitY.RotatedBy((double)(Projectile.localAI[0] * 0.1308997f + i * 3.14159274f), default) * rotateVector;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            {
                texture = CIProjectiles_Melee.EntropicFlechette1.Value;
            }
            else if (Projectile.ai[0] == 1)
            {
                texture = CIProjectiles_Melee.EntropicFlechette2.Value;
            }
            else
            {
                texture = CIProjectiles_Melee.EntropicFlechette3.Value;
            }
            LAPUtilities.DrawAfterimages(Projectile, ProjectileID.Sets.TrailingMode[Projectile.type], lightColor, 2, texture);
            return false;
        }
    }
}
