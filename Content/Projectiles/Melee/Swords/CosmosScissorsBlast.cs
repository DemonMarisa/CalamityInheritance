...
{
    public class CosmosScissorsBlast : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            if (HoldTimer == 1)
            {
                ...
                {
                    //60帧，确保这个大撕裂冲刺期间可以保证覆盖
                    Owner.GiveIFrames(1, 60);
                    Dashing = true;
                ...
                if (Owner.Distance(scissorPosition) < 60f)
                {
                    ...
                    if (Owner.whoAmI == Main.myPlayer)
                    {
                        ...
                        for (int i = 0; i < 7; i++)
                        {
                            ...
                            Projectile blast = Projectile.NewProjectileDirect(source, Owner.Center, Main.rand.NextVector2CircularEdge(28, 28), ProjectileType<EonBolt>(), (int)(CosmosScissors.SlashBoltsDamageMultiplier * Projectile.damage), 0f, Owner.whoAmI, 0.55f, MathHelper.Pi * 0.07f);
                            {
                                blast.timeLeft = 100;
                            }
                        }
        ...
        public void HandleParticles()
        {
            ...
            for (int i = 0; i < CurrentStitches; i++)
            {
                if (StitchRotations[i] == 0)
                {
                    ...
                    float positionAlongLine = ThrustDisplaceRatio() * 242f / maxStitches * 0.5f + MathHelper.Lerp(0f, ThrustDisplaceRatio() * 242f, i / (float)maxStitches);
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            ...
            modifiers.SourceDamage *= (float)Math.Pow(1 - CosmosScissors.blastFalloffStrenght, Projectile.numHits * CosmosScissors.blastFalloffSpeed);
            //保证暴击
            modifiers.SetCrit();
        }
