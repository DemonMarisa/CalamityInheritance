...
{
    public class CosmosScissorsParry: ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            if (!initialized) //Initialization
            {
                ...
                Projectile.velocity = Owner.SafeDirectionTo(Owner.LocalMouseWorld(), Vector2.Zero);
                   ...
                   Collision.CheckAABBvLineCollision(proj.Hitbox.TopLeft(), proj.Hitbox.Size(), Owner.Center + DistanceFromPlayer, Owner.Center + DistanceFromPlayer + Projectile.velocity * bladeLength, 24, ref collisionPoint))
                {
                    if (AlreadyParried == 0)
                    {
                        ...
                        if (Owner.velocity.Y != 0)
                            Owner.velocity += (Owner.Center - proj.Center).SafeNormalize(Vector2.Zero) * 2;
                    }
