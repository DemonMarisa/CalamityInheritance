...
{
    public class CosmosScissorsSwingBlade : ModProjectile, ILocalizedModType
    {
        ...
        private bool OwnerCanShoot => Owner.channel && !Owner.noItems && !Owner.CCed && Owner.HeldItem.type == ItemType<CosmosScissors>();
        ...
        public override void AI()
        {
            ...
            if (!Thrown)
            {
                ...
                else
                {
                    ...
                    if (Owner.whoAmI == Main.myPlayer && (Projectile.timeLeft - 1) % Math.Ceiling(MaxSwingTime / CosmosScissors.SwirlBoltAmount) == 0f)
                    {
            ...
            else
            {
                ...
                if (Math.Abs(ThrowCompletion - SnapWindowStart + 0.1f) <= 0.005f && ChanceMissed == 0f && Main.myPlayer == Owner.whoAmI)
                {
                    ...
                    var source = Projectile.GetSource_FromThis();
                    Projectile chain = Projectile.NewProjectileDirect(source, Owner.Center, Vector2.Zero, ProjectileType<CosmosScissorsConstellation>(), (int)(Projectile.damage * CosmosScissors.chainDamageMultiplier), 0, Owner.whoAmI, (int)(Projectile.timeLeft / 2f));
                ...
                if ((Projectile.Center - Owner.Center).Length() > CosmosScissors.MaxThrowReach)
                    Projectile.Center = Owner.Center + Owner.DirectionTo(Projectile.Center) * CosmosScissors.MaxThrowReach;
                ...
                {
                    Particle snapSpark = new GenericSparkle(Projectile.Center, Owner.velocity - Projectile.velocity.SafeNormalize(Vector2.Zero), Color.White, Color.OrangeRed, Main.rand.NextFloat(1f, 2f), 10 + Main.rand.Next(10), 0.1f, 3f);
                    ...
                    if (Owner.whoAmI == Main.myPlayer)
                    {
                        ...
                        {
                            var source = Projectile.GetSource_FromThis();
                            Projectile blast = Projectile.NewProjectileDirect(source, Projectile.Center + (MathHelper.TwoPi * (i / 3f) + rotationOffset).ToRotationVector2() * 30f, (MathHelper.TwoPi * (i / 3f) + rotationOffset).ToRotationVector2() * 20f, ProjectileType<EonBolt>(), (int)(CosmosScissors.SnapBoltsDamageMultiplier * Projectile.damage), 0f, Owner.whoAmI, 0.55f, MathHelper.Pi * 0.05f);
                            {
                                blast.timeLeft = 100;
                            }
                        }
        ...
        public void DoParticleEffects(bool swirlSwing)
        {
            if (swirlSwing)
            {
                ...
                if (Main.rand.NextBool())
                {
                    ...
                    Vector2 angularVelocity = distance.RotatedBy(MathHelper.PiOver2 * Owner.direction).SafeNormalize(Vector2.Zero) * 2 * (1f + distance.Length() / 15f);
            ...
            else
            {
                ...
                if (Combo == 2f)
                {
                    if (Main.rand.NextBool())
                    {
                        float maxDistance = Projectile.scale * 78f;
                        Vector2 distance = Main.rand.NextVector2Circular(maxDistance, maxDistance);
                        Vector2 angularVelocity = distance.RotatedBy(-MathHelper.PiOver2).SafeNormalize(Vector2.Zero) * 2 * (1f + distance.Length() / 15f);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 5; i++)
            {
                Vector2 particleSpeed = (target.Center - Projectile.Center).SafeNormalize(Vector2.One).RotatedByRandom(MathHelper.PiOver4 * 0.8f) * Main.rand.NextFloat(3.6f, 8f);
            ...
            if (Combo == 3f)
            {
                ...
                if (Charge <= 1)
                {
                    CosmosScissors sword = Owner.HeldItem.ModItem as CosmosScissors;
                    if (sword != null)
                        sword.Charge = 2f;
                }
            }
        ...
        public override void OnKill(int timeLeft)
        {
            if (Combo == 3f)
            {
                ...
                Vector2 sliceDirection = direction.SafeNormalize(Vector2.One) * 40;
        ...
        public void DrawSingleSwungScissorBlade(Color lightColor)
        {
            ...
            if (CalamityClientConfig.Instance.Afterimages && SwingTimer > ProjectileID.Sets.TrailCacheLength[Projectile.type] && Combo == 0f)
            {
                for (int i = 1; i < Projectile.oldRot.Length; ++i)
                {
                    ...
                    SB.Draw(glowmask, drawOffset, null, color * 0.05f, afterimageRotation, drawOrigin, Projectile.scale - 0.2f * (i / (float)Projectile.oldRot.Length), flip, 0f);
                }
            }
            ...
            if (SwingCompletion > 0.5f && Combo == 0f)
            {
                ...
                SB.End();
                SB.Begin(SpriteSortMode.Immediate, BlendState.Additive, Main.DefaultSamplerState, DepthStencilState.None, Main.Rasterizer, null, Main.GameViewMatrix.TransformationMatrix);
