...
{
    public class VirulentBeeWave : GeneralDamageProj
    {
            ...
            if (Projectile.ai[1] < 60f)
            {
                if (Projectile.ai[0] > 7f)
                {
                    ...
                    if (Projectile.ai[0] % 2f == 0f)
                    {
                        ...
                        int bee = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X + (float)Main.rand.Next(-spawnX, spawnX), Projectile.Center.Y + (float)Main.rand.Next(-spawnY, spawnY), Projectile.velocity.X * 0.5f, Projectile.velocity.Y * 0.5f,
                            ProjectileType<PlagueBeeSmall>(), player.beeDamage(Projectile.damage / 3), player.beeKB(0f), Projectile.owner);
