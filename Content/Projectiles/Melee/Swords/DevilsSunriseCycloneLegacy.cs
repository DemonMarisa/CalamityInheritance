...
{
    public class DevilsSunriseCycloneLegacy : CIMeleeProj
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<DevilsSunriseLegacy>();
        private int red = 0;
        ...
        public override void AI()
        {
            ...
            float x = Owner.position.X + Owner.width / 2 - projDirection.X;
            float y = Owner.position.Y + Owner.height / 2 - projDirection.Y;
            ...
            else if (Main.myPlayer == Projectile.owner && Projectile.ai[0] <= 0f)
            {
                if (Owner.channel)
                {
                    ...
                    float mouseDestX = Main.mouseX + Main.screenPosition.X - projTravel.X;
                    float mouseDestY = Main.mouseY + Main.screenPosition.Y - projTravel.Y;

                    if (Owner.gravDir == -1f)
                        mouseDestY = Main.screenPosition.Y + Main.screenHeight - Main.mouseY - projTravel.Y;
                ...
                else if (Projectile.ai[0] <= 0f)
                {
                    ...
                    float miceX = Main.mouseX + Main.screenPosition.X - projDirect.X;
                    float miceY = Main.mouseY + Main.screenPosition.Y - projDirect.Y;

                    if (Owner.gravDir == -1f)
                        miceY = Main.screenPosition.Y + Main.screenHeight - Main.mouseY - projDirect.Y;

                    float miceDistancing = (float)Math.Sqrt((double)(miceX * miceX + miceY * miceY));
                    if (miceDistancing == 0f || Projectile.ai[0] < 0f)
                    {
                        projDirect = new Vector2(Owner.position.X + Owner.width / 2, Owner.position.Y + Owner.height / 2);
                        miceX = Projectile.position.X + Projectile.width * 0.5f - projDirect.X;
                        miceY = Projectile.position.Y + Projectile.height * 0.5f - projDirect.Y;
            ...
            Lighting.AddLight(Projectile.Center, (float)(red * 0.001), 0.1f, 0.1f);
        ...
        {
            target.AddBuff(BuffType<CIBrimstoneFlames>(), 180);
        }
    }
}
