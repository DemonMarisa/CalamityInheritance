...
{
    public class PulseRifleShotOld : CIRangedProj
    {
        ...
        public override void ModifyHitNPC(NPC target, ref NPC.HitModifiers modifiers)
        {
            if (Projectile.ai[1] < 5f && !hasHit && Main.myPlayer == Projectile.owner)
            {
                ...
                for (int i = 0; i < Main.maxNPCs; i++)
                {
                    ...
                    if (hasTar != Main.npc[i].whoAmI && (Projectile.Center - Main.npc[i].Center).Length() < 600f)
                    {
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, LAPUtilities.GetVector2(Projectile.Center, Main.npc[i].Center) * 5f, Projectile.type, Projectile.damage / 2, 0f, Projectile.owner, Main.npc[i].whoAmI, Projectile.ai[1] + 1f);
                        break;
                    }
                }
            }
