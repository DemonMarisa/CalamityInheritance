...
{
    public class TaserProj : CIRangedProj
    {
        ...
        public override void AI()
        {
            ...
            switch (AIState)
            {
                ...
                case TaserAIState.ReelingBack:
                    ...
                    Projectile.velocity = LAPUtilities.GetVector2(Projectile.Center, player.Center) * ReelbackSpeed;
                    break;
            }
        ...
        public void ManipulatePlayerItemValues(Player player)
        {
            ...
            player.itemRotation = WrapAngle90Degrees(Projectile.rotation);
            player.itemTime = 4;
            ...
            if (AIState == TaserAIState.Firing)
            {
                if (!Main.dedServ)
                {
                    for (int i = 0; i < 50; i++)
                    {
                        ...
                        Dust dust = Dust.NewDustPerfect(target.Center + angle.ToRotationVector2() * 10f, DustID.Electric);
                        dust.velocity = Vector2.Zero;
                        if (Main.rand.NextBool(6))
                            dust.velocity = LAPUtilities.GetVector2(target.Center, dust.position) * 4.5f;
