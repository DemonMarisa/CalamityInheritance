...
namespace CalamityInheritance.Content.Projectiles.DraedonsArsenal.Magic
{
    public class GatlingLaserShot : CIMagicProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft % 11 == 0 && Time > 15)
            {
                new SparkParticle(Projectile.Center, Projectile.velocity * 0.01f, false, 12, 1.7f, Color.Red).Spawn();
            }
            if (Projectile.timeLeft % 4 == 0 && Time > 15)
            {
                new SparkParticle(Projectile.Center, Projectile.velocity * 0.01f, false, 12, 0.4f, Color.White).Spawn();
            }
        }
        ...
        private void LaserBurst(float speed1, float speed2)
        {
            ...
            for (int j = 0; j < 4; j = inc + 1)
            {
                ...
                int burstDust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, dust, dustVel.X, dustVel.Y, 200, default, 0.9f);
                ...
                burstDust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, dust, dustVel.X, dustVel.Y, 100, default, 0.66f);
                Main.dust[burstDust].position = Projectile.Center + Vector2.UnitY.RotatedByRandom(MathHelper.Pi) * (float)Main.rand.NextDouble() * Projectile.width / 2f;
