...
{
    public class FrequencyManipulatorProjectile : CIRogueProj
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<FrequencyManipulator>();
        public override string Texture => GetInstance<FrequencyManipulator>().Texture;
        public float Time
        {
            get => Projectile.ai[0];
        ...
        public override void AI()
        {
            ...
            else if (Time == SpinTime + 1f)
            {
                ...
                Projectile.Center = Projectile.Owner().RotatedRelativePoint(Projectile.Owner().MountedCenter, true);
                Projectile.velocity = Projectile.Owner().GetToMouseVector2(Projectile.Center) * player.ActiveItem().shootSpeed;
                Projectile.rotation = Projectile.velocity.ToRotation() + MathHelper.PiOver4;
            }
        }
        ...
        public void RingDustEffect(Vector2 spawnPosition, float scale, float fadeIn, float ringSpeed, float secondaryRingSpeedMultiplier)
        {
            ...
            for (int i = 0; i < 36; i++)
            {
                ...
                Dust dust = Dust.NewDustPerfect(spawnPosition, DustID.BoneTorch);
                dust.scale = scale;
                dust.fadeIn = scale;
                dust.velocity = angle.ToRotationVector2() * ringSpeed;

                dust = Dust.NewDustPerfect(spawnPosition, DustID.ShadowbeamStaff);
                dust.scale = scale;
                dust.fadeIn = scale;
        ...
        public void OnHitEffects()
        {
            ...
            for (int i = 0; i < totalEnergyParticlesToSpawn; i++)
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center + velocity * 1.8f, velocity, ProjectileType<FrequencyManipulatorEnergy>(), Projectile.damage, Projectile.knockBack * 0.4f, Projectile.owner);
            }
        }
