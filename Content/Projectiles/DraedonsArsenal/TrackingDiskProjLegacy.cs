...
{
    public class TrackingDiskProj : RogueDamageProj, ILocalizedModType
    {
        ...
        public void AttemptToFireLasers(int damage)
        {
            ...
            if (Projectile.Calamity().stealthStrike)
            {
                ...
                foreach (var target in targets)
                {
                    if (targetCount >= MaxLaserCountPerShot)
                        break;
                    Projectile laser = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.SafeDirectionTo(target.Center) * 4f, ProjectileType<TrackingDiskLaserProj>(), (int)(damage * 0.6f), Projectile.knockBack, Projectile.owner, 1f);
                    laser.scale *= 1.6f;
            ...
            else
            {
                ...
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.SafeDirectionTo(potentialTarget.Center) * 3f, ProjectileType<TrackingDiskLaserProj>(), damage, Projectile.knockBack, Projectile.owner);
                }
            }
        }
