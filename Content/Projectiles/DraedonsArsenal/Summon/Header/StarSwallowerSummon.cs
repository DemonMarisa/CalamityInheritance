...
namespace CalamityInheritance.Content.Projectiles.DraedonsArsenal.Summon.Header
{
    public class StarSwallowerSummon : CISummonProj
    {
        ...
        public override void AI()
        {
            ...
            bool isCorrectProjectile = Projectile.type == ProjectileType<StarSwallowerSummon>();
            player.AddBuff(BuffType<StarSwallowerBuff>(), 2);
            ...
            if (potentialTarget != null &&
                Math.Abs(potentialTarget.Center.X - Projectile.Center.X) < 640f &&
                Math.Sign(potentialTarget.Center.Y - Projectile.Center.Y) == -1 &&
                Math.Abs(potentialTarget.Center.Y - Projectile.Center.Y) < 640f &&
                Math.Sign(potentialTarget.Center.Y - Projectile.Center.Y) == -1 &&
                !FlyUntilNearPlayer)
            {
        ...
        public void PlayerSeparationAnxietyAI(Player player, ref int minimumFrame, ref int maximumFrame)
        {
            ...
            if (flying)
            {
                ...
                maximumFrame = 11;
                float inertia = 30f;
                Projectile.velocity = (Projectile.velocity * (inertia - 1) + LAPUtilities.GetVector2(Projectile.Center, player.Center) * 18f) / inertia;
            }
        ...
        public void NPCTargetingAI(NPC potentialTarget)
        {
            ...
            if (Collision.CanHit(Projectile.position, Projectile.width, Projectile.height, potentialTarget.position, potentialTarget.width, Projectile.height) && AcidShootCooldown <= 0f)
            {
                    ...
                    Main.myPlayer == Projectile.owner)
                {
                    ...
                    Vector2 fireVelocity = CIUtils.GetProjectilePhysicsFiringVelocity(spawnPosition, potentialTarget.Center, StarSwallowerAcid.Gravity, 14f);

                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), spawnPosition, fireVelocity, ProjectileType<StarSwallowerAcid>(),
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Main.EntitySpriteDraw(texture, Projectile.Center - Main.screenPosition, frame, lightColor, Projectile.rotation, Projectile.Size * 0.5f, Projectile.scale,
