...
{
    public class LunicBeamLegacy : CITypelessProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.alpha == 0)
            {
                for (int j = 0; j < 2; j++)
                {
                    ...
                    rotateFirstDust = -Vector2.UnitY.RotatedBy((double)(Projectile.localAI[0] * 0.1308997f + j * 3.14159274f), default) * dustRotation - Projectile.rotation.ToRotationVector2() * 10f;
            ...
            if (Main.rand.NextBool(12))
            {
                for (int k = 0; k < 1; k++)
                {
                    ...
                    Main.dust[smokyDust].position = Projectile.Center + rotateSecondDust * Projectile.width / 2f + Projectile.velocity * 2f;
            ...
            if (Main.rand.NextBool(64))
            {
                for (int l = 0; l < 1; l++)
                {
                    ...
                    Main.dust[smokyDust2].position = Projectile.Center + rotateThirdDust * Projectile.width / 2f;
            ...
            if (Main.rand.NextBool(4))
            {
                for (int m = 0; m < 2; m++)
                {
                    ...
                    Main.dust[randomDust].position = Projectile.Center + rotateFourthDust * Projectile.width / 2f;
                    if (Main.rand.NextBool())
                    {
            ...
            if (Main.rand.NextBool(3))
            {
                ...
                Main.dust[solarDust2].position = Projectile.Center + rotateFifthDust * Projectile.width / 2f;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) => target.AddBuff(BuffType<CIMarkedforDeath>(), 480);

        public override void OnHitPlayer(Player target, Player.HurtInfo info) => target.AddBuff(BuffType<CIMarkedforDeath>(), 480);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int j = 0; j < 40; j = inc + 1)
            {
                ...
                dust.position = Projectile.Center + Vector2.UnitY.RotatedByRandom(3.1415927410125732) * (float)Main.rand.NextDouble() * Projectile.width / 2f;
                dust.noGravity = true;
                dust.velocity *= 3f;
            ...
            for (int k = 0; k < 20; k = inc + 1)
            {
                ...
                dust.position = Projectile.Center + Vector2.UnitX.RotatedByRandom(3.1415927410125732).RotatedBy((double)Projectile.velocity.ToRotation(), default) * Projectile.width / 3f;
                dust.noGravity = true;
                dust.velocity *= 0.5f;
