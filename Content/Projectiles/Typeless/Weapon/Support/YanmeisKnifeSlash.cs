...
{
    public class YanmeisKnifeSlash : CITypelessProj
    {
        ...
        {
            target.AddBuff(BuffType<KamiFlu>(), 600);
            if (!Main.dedServ)
            {
                for (int i = 0; i < 60; i++)
                {
            ...
            {
                Player player = Main.player[Projectile.owner];
                SoundEngine.PlaySound(CISounds.YanmeiKnifeHit, Projectile.position);
                SoundEngine.PlaySound(CISounds.YanmeiKnifeExpire, player.Center);
                Projectile.ai[0] = 1f;
            }
            Main.player[Projectile.owner].AddBuff(BuffType<KamiBuff>(), 600);
        }
