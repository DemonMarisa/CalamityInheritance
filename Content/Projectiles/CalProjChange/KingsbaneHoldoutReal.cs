...
{
    public class KingsbaneHoldoutReal : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (OwnerCanShoot)
            {
                if (Time < 30 && soundTimer > (windupAnim + 2))
                {
                ...
                if (Time > 30)
                {
                    ...
                    if (framesBetweenShots == 0)
                    {
                        ...
                        Projectile proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Owner.Center, shootVelocity.RotatedByRandom(MathHelper.ToRadians(3f)), bulletAMMO, Projectile.damage, Projectile.knockBack, Projectile.owner);
                        if (proj != null)
                        {
                            //proj.penetrate = 2;
