...
namespace CalamityInheritance.Content.Projectiles.CalProjChange
{
    public class PhotovisceratorLaser : ModProjectile, ILocalizedModType
    {
        ...
        public void DrawLaserBeam()
        {
            ...
            float Scale = Projectile.ai[2] == 1 ? 0.3f : 1.5f * Projectile.ai[0];
            // 颜色
            ...
            {
                DrawMainBeam(mainTexture, baseColor, beamRotation, laserLength, Scale, Vector2.Zero);
                DrawMainBeam(mainTexture, baseColor, beamRotation, laserLength, Scale, new Vector2(0, -8f));

                DrawBloomEffect(bloomTexture, Auxiliarycolor, beamRotation, laserLength, Scale, new Vector2(0, -8f));
        ...
        #endregion
        #region 绘制本体辉光
        public void DrawBloomEffect(Texture2D texture, Color color, float rotation, int length, float Scale, Vector2 offset)
        {
        ...
        #endregion
        #region 绘制主光束
        private void DrawMainBeam(Texture2D texture, Color color, float rotation, int length, float Scale, Vector2 offset)
        {
            Rectangle rect = new Rectangle(0, 0, length, texture.Height);
