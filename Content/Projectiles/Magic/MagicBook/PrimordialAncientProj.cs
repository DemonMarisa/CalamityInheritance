...
{
    public class PrimordialAncientProj : CIMagicProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft % 30 == 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    for (int i = -1; i < numProj - 1; i++)
                    {
                        ...
                        Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, perturbedSpeed, ProjectileType<Ancient2>(), (int)(Projectile.damage * 0.5), Projectile.knockBack * 0.5f, Projectile.owner, 0f, Projectile.ai[1]);
                    }
                }
            }
            ...
            if (Projectile.FinalExtraUpdate())
            {
                ...
                {
                    // Dust
                    Vector2 dustPos = Projectile.Center + new Vector2(Projectile.width / 2, 0).RotatedByRandom(MathHelper.TwoPi);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) => target.AddBuff(BuffType<CIArmorCrunch>(), 180);

        public override void OnHitPlayer(Player target, Player.HurtInfo info) => target.AddBuff(BuffType<CIArmorCrunch>(), 180);
