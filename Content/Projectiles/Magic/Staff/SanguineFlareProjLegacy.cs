...
using CalamityInheritance.Content.Items.Weapons.Magic.Staff;
...
{
    public class SanguineFlareProjLegacy : CIMagicProj
    {
        private int x;
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                int brimDust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.LifeDrain, 0f, 0f, 100, default, 1.2f);
            ...
            for (int j = 0; j < 6; j++)
            {
                int brimDust2 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.LifeDrain, 0f, 0f, 100, default, 1.7f);
                Main.dust[brimDust2].noGravity = true;
                Main.dust[brimDust2].velocity *= 5f;
                brimDust2 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.LifeDrain, 0f, 0f, 100, default, 1f);
        ...
        {
            int heal = (int)Math.Round(Projectile.Owner().GetIntDamage<MagicDamageClass>(SanguineFlareLegacy.BaseDamage) * 0.025);
