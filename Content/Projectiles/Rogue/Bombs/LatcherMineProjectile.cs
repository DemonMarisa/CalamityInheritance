...
{
    public class LatcherMineProjectile : CIRogueProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[0] == 1f)
            {
                ...
                int secondsToLive = Projectile.CI().Stealth ? 14 : 6;
            ...
            if (Projectile.timeLeft == 110 * (Projectile.CI().Stealth ? 2 : 1) ||
                Projectile.timeLeft == 60 * (Projectile.CI().Stealth ? 2 : 1) ||
                Projectile.timeLeft == 24 * (Projectile.CI().Stealth ? 2 : 1))
            {
                Projectile.frame++;
                Projectile.netUpdate = true;
            }
            if (Projectile.timeLeft < 24 * (Projectile.CI().Stealth ? 2 : 1))
            {
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            Projectile.width = Projectile.height = Projectile.CI().Stealth ? 240 : 100;
            Projectile.position.X = Projectile.position.X - Projectile.width / 2;
            Projectile.position.Y = Projectile.position.Y - Projectile.height / 2;
            Projectile.Damage();
            for (int i = 0; i < (Projectile.CI().Stealth ? 7 : 4); i++)
            {
                if (Main.rand.NextBool(2) && Projectile.CI().Stealth)
                {
                ...
                else
                {
                    ...
                    int fireIndex = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Top, fireVelocity, ProjectileType<FlameLegacy>(), projdmg / 3, 1f, Projectile.owner);
