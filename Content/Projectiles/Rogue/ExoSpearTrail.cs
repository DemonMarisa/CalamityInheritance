        ...
        public override void AI()
        {
            ...
            if (timer > 20)
            {
                for (int i = 0; i < 200; i++)
                {
                    ...
                    if (distance < maxDistance && possibleTarget.active && !possibleTarget.dontTakeDamage && possibleTarget.chaseable && !possibleTarget.friendly && possibleTarget.lifeMax > 5 && !possibleTarget.immortal && Collision.CanHit(Projectile.Center, 0, 0, ((Entity)possibleTarget).Center, 0, 0))
                    {
                        target = Main.npc[i];
                        foundTarget = true;
