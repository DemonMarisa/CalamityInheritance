...
{
    public class RogueScourgeoftheCosmosProj : ModProjectile, ILocalizedModType
    {
        public new string LocalizationCategory => "Content.Projectiles.Rogue";
        public override string Texture => GetInstance<RogueScourgeoftheCosmos>().Texture;
        public int bounce = 2;
                ...
                else
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        for (int j = 0; j < minisAmt; j = inc + 1)
                        {
                            ...
                            randXDirect *= 10f;
                            randYDirect *= 10f;
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, randXDirect, randYDirect, ProjectileType<RogueScourgeoftheCosmosEater>(), projectileDamage, Projectile.knockBack * 0.35f, Main.myPlayer, 5);
                            inc = j;
                        }
                    }
                }
                ...
                else
                {
                    ...
                    if (Projectile.owner == Main.myPlayer)
                    {
                        ...
                        for (int j = 0; j < minisAmt; j = inc + 1)
                        {
                            ...
                            randXDirect *= 10f;
                            randYDirect *= 10f;
                            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, randXDirect, randYDirect, ProjectileType<RogueScourgeoftheCosmosEaterClone>(), projectileDamage, Projectile.knockBack * 0.35f, Main.myPlayer);
                            inc = j;
                        }
                    }
                }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (Projectile.owner == Main.myPlayer)
            {
                ...
                for (int j = 0; j < minisAmt; j = inc + 1)
                {
                    ...
                    randXDirect *= 10f;
                    randYDirect *= 10f;
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position.X, Projectile.position.Y, randXDirect, randYDirect, ProjectileType<RogueScourgeoftheCosmosEater>(), projectileDamage, Projectile.knockBack * 0.35f, Main.myPlayer);
                    inc = j;
                }
            }
        }
