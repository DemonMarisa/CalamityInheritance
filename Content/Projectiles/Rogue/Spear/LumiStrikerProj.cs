...
{
    public class LumiStrikerProj : CIRogueProj
    {
        ...
        public void NorAI(float rotOffset)
        {
            ...
            //生成月明碎片
            if (Projectile.timeLeft % 20 == 0 && Projectile.owner == Main.myPlayer && (Projectile.Center - Projectile.Owner().Center).Length() < 1000)
        ...
        public void StealthAI(float rotOffset)
        {
            ...
            {
                NPC target = LAPUtilities.FindClosestTarget(Projectile.Center, 2000);
        ...
        public void DoBehavior_Charge(NPC npc, ref float attacktimer, float rotOffset)
        {
            ...
            {
                Projectile.rotation = Projectile.velocity.ToRotation() + rotOffset;
                Projectile.HomingTarget( npc.Center, 2000, 12f, 0f);
            }
            // 这里代表冲刺过程
            if (hasCharge == false)
            {
