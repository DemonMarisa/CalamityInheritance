...
{
    public class MeteorFistProjLegacy : CIRogueProj
    {
        ...
        public override void AI()
        {
            if (Projectile.velocity.Length() >= 4f)
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    Main.dust[meteorDust].scale *= 2f + Main.rand.Next(10) * 0.1f;
            ...
            else if (Main.rand.NextBool())
            {
                ...
                Main.dust[fieryDust].scale = 0.1f + Main.rand.Next(5) * 0.1f;
                Main.dust[fieryDust].fadeIn = 1.5f + Main.rand.Next(5) * 0.1f;
                Main.dust[fieryDust].noGravity = true;
                Main.dust[fieryDust].position = Projectile.Center + new Vector2(0f, (float)(-(float)Projectile.height / 2)).RotatedBy(Projectile.rotation, default) * 1.1f;
                Main.rand.Next(2);
                fieryDust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Torch, 0f, 0f, 100, default, 0.5f);
                Main.dust[fieryDust].scale = 1f + Main.rand.Next(5) * 0.1f;
                Main.dust[fieryDust].noGravity = true;
                Main.dust[fieryDust].position = Projectile.Center + new Vector2(0f, (float)(-(float)Projectile.height / 2 - 6)).RotatedBy(Projectile.rotation, default) * 1.1f;
            }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int j = 0; j < 40; j++)
            {
                ...
                if (Main.rand.NextBool())
                {
                    ...
                    Main.dust[boomDust].fadeIn = 1f + Main.rand.Next(10) * 0.1f;
                }
            }
            for (int k = 0; k < 70; k++)
            {
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ProjectileType<FuckYou>(), Projectile.damage, Projectile.knockBack, Projectile.owner, 0f, 0.85f + Main.rand.NextFloat() * 1.15f);
        }
