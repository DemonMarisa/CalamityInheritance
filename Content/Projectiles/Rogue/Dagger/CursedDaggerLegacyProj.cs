...
using CalamityInheritance.Content.Items.Weapons.Rogue.Dagger;
...
namespace CalamityInheritance.Content.Projectiles.Rogue.Dagger
{
    //要是想重置下特效也行，不过我暂时找不到这武器需要重置特效的必要
    //这里有一个基于射弹速度修改生成频率时间戳的方式。用来尽可能解决射弹速度越快反而会导致总衍生射弹量减少的问题
    public class CursedDaggerProjLegacy : CIRogueProj
    {
        public override string Texture => GetInstance<CursedDaggerLegacy>().Texture;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Projectile.BaseProjPreDraw(tex, lightColor);
            return false;
        }
        //基础生成间隔
        ...
        public override void AI()
        {
            ...
            if (!Projectile.CI().Stealth && Projectile.owner == Main.myPlayer)
            {
                if (Projectile.timeLeft % spawnRates == 0)
                {
                    ...
                    Main.projectile[flame].DamageType = RogueDamage.Instance;
