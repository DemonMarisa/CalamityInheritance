...
{
    public class ExorcismProjLegacy : CIRogueProj
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<ExorcismLegacy>();
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            for (int i = 0; i < 6; i++)
            {
                ...
                Vector2 starVelocity = LAPUtilities.GetVector2(pos, target.Center) * 30f;
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), pos, starVelocity, ProjectileType<ExorcismStarLegacy>(), (int)(Projectile.damage * 0.75f), Projectile.knockBack * 0.5f, Projectile.owner, Main.rand.NextFloat(-3f, 3f), 0f);
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            // Light burst
            int p = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ProjectileType<ExorcismShockwave>(), (int)(Projectile.damage * 0.8f), 0, Projectile.owner, Projectile.ai[0], 0);
