...
{
    public class SearedPanProjectile : CIRogueProj
    {
        ...
        {
            OnHitEffects(target.whoAmI, target.life, true);
        }
        private void OnHitEffects(int targetIndex, int health, bool specialEffects)
        {
            ...
            if (specialEffects && PanType != SearedPanTypes.VenLocket)
                SearedPan.searedPanCounter++;

            // Pans summoned via the Venerated Locket have zero special effects
            if (PanType == SearedPanTypes.StealthStrike)
            {
                SearedPan.searedPanCounter = 0;
                // Stealth strikes spawn four golden sparks on hit
                for (int t = 0; t < 4; t++)
                {
                    Vector2 velocity = CIUtils.RandomVelocity(10f, 7f, 10f);
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, velocity, ProjectileType<FlameLegacy>(), (int)(Projectile.damage * 0.2), 0f, Projectile.owner);
                }
                SoundEngine.PlaySound(CISounds.SearedPanSmash, Projectile.position);
                ...
                for (int t = 0; t < 5; t++)
                {
                    int i = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ProjectileType<NiceCock>(), (int)(Projectile.damage * 0.1), 0f, Projectile.owner, 0f, targetIndex);
            ...
            else if (PanType == SearedPanTypes.Normal && targetIndex != -1 && health > 0 && specialEffects)
            {
                ...
                for (int t = 0; t < 3; t++)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Vector2.Zero, ProjectileType<NiceCock>(), (int)(Projectile.damage * 0.1), 0f, Projectile.owner, 0f, targetIndex);
                }
                FireballPositions(targetIndex);
            }
        }
