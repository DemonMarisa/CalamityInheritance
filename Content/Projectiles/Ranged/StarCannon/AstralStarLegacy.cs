...
namespace CalamityInheritance.Content.Projectiles.Ranged.StarCannon
{
    public class AstralStarLegacy : CIRangedProj
    {
        public override string Texture => CITextureRegister.StarTrail.Path;
        ...
        public override void AI()
        {
            ...
            if (Main.rand.NextBool(16))
            {
                ...
                int astralDust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustType<CIAstralOrange>(), Projectile.velocity.X * 0.5f, Projectile.velocity.Y * 0.5f, 150, default, 1f);
            ...
            if (Projectile.ai[1] == 1f)
            {
                ...
                if (Main.rand.NextBool(10))
                    Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustType<CIAstralOrange>(), Projectile.velocity.X * 0.5f, Projectile.velocity.Y * 0.5f, 150, default, 1f);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Projectile.DamageType != DamageClass.Ranged)
                target.AddBuff(BuffType<CIAstralInfection>(), 120);
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                int starryDust = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustType<CIAstralOrange>(), 0f, 0f, 100, default, 1.2f);
            ...
            for (int j = 0; j < 5; j++)
            {
                int starryDust2 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustType<CIAstralOrange>(), 0f, 0f, 100, default, 1.7f);
                Main.dust[starryDust2].noGravity = true;
                Main.dust[starryDust2].velocity *= 5f;
                starryDust2 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustType<CIAstralOrange>(), 0f, 0f, 100, default, 1f);
