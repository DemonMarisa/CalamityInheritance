...
namespace CalamityInheritance.Content.Projectiles.Ranged.DartRifle
{
    public class ChargedBlastLegacy : CIRangedProj
    {
        ...
        public override void AI()
        {
            ...
            {
                LineParticle spark = new LineParticle(Projectile.Center - Projectile.velocity * 3, -Projectile.velocity * 0.05f, false, 5, 2f, baseColor * 0.65f);
                spark.Spawn();
            }
        }
        public override void OnKill(int timeLeft)
        {
            if (!outOfTime)
            {
                ...
                for (int k = 0; k < 3; k++)
                {
                    LineParticle spark2 = new LineParticle(Projectile.Center, (-baseVel * 4).RotatedByRandom(0.9f) * Main.rand.NextFloat(0.8f, 1.2f), false, Main.rand.Next(25, 32 + 1), Main.rand.NextFloat(1.5f, 2f), baseColor);
                    spark2.Spawn();
                }
                if (Projectile.ai[2] == 1)
                {
                    LineParticle spark2 = new LineParticle(Projectile.Center, (-baseVel * 5).RotatedByRandom(0.9f) * Main.rand.NextFloat(0.8f, 1.2f), false, Main.rand.Next(35, 48 + 1), Main.rand.NextFloat(2.3f, 3f), baseColor);
                    spark2.Spawn();
                }
            }
        }
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            LAPUtilities.DrawAfterimages(Projectile, ProjectileID.Sets.TrailingMode[Projectile.type], (baseColor * 0.7f) with { A = 0 }, 1, texture);
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) => LAPUtilities.CircularHitboxCollision(Projectile.Center, 20, targetHitbox);
    }
}
