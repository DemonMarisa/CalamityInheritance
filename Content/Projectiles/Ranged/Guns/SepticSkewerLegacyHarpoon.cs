...
{
    public class SepticSkewerLegacyHarpoon : CIRangedProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.ai[1] % 8f == 0f && Projectile.owner == Main.myPlayer && Main.rand.NextBool(5))
            {
                ...
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, harpoonPos.X, harpoonPos.Y, ProjectileType<SepticSkewerLegacyBacteria>(), (int)(Projectile.damage * 0.175), Projectile.knockBack * 0.2f, Projectile.owner, -10f, 0f);
            }
            if (Owner.dead)
            {
                Projectile.Kill();
            ...
            float xDist = Owner.position.X + (float)(Owner.width / 2) - halfDist.X;
            float yDist = Owner.position.Y + (float)(Owner.height / 2) - halfDist.Y;
