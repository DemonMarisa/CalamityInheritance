...
{
    public class TriploonProj : CIRangedProj
    {
        ...
        public override void AI()
        {
            ...
            float xDist = player.position.X + player.width / 2 - halfDist.X;
            float yDist = player.position.Y + player.height / 2 - halfDist.Y;
            float playerDistance = (float)Math.Sqrt((double)(xDist * xDist + yDist * yDist));
            if (Projectile.ai[0] == 0f)
            {
                if (playerDistance > 1800f)
                {
                    Projectile.ai[0] = 1f;
                }
