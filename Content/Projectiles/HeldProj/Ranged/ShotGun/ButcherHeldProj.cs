...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Ranged.ShotGun
{
    public class ButcherHeldProj : CIRangedProj
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<ButcherLegacy>();
        ...
        public override void AI()
        {
            ...
            {
                Projectile.ai[1] = shootDelayBase - incrementMult * incrementAmt;
                willShoot = true;
            }
            ...
            if (willShoot && Main.myPlayer == Projectile.owner)
            {
                ...
                if (canShoot)
                {
                    ...
                    {
                        targetPos.Y = Owner.LocalMouseWorld().Y + Main.screenPosition.Y - source.Y;
                    }
                    ...
                    for (int projIndex = 0; projIndex < randomBulletCount; projIndex++)
                    {
                        ...
                        bulletVel.X += Main.rand.Next(-15, 16) * spreadMult;
                        bulletVel.Y += Main.rand.Next(-15, 16) * spreadMult;
