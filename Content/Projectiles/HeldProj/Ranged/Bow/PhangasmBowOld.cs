...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Ranged.Bow
{
    public class PhangasmBowOld : ModProjectile, ILocalizedModType
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<Phangasm>();
        public override string Texture => GetInstance<Phangasm>().Texture;
        ...
        public override void AI()
        {
            ...
            {
                Projectile.ai[1] = delayCompare - fireSpeedCompare * fireSpeed;
                fullSpeed = true;
            ...
            if (fullSpeed && Main.myPlayer == Projectile.owner)
            {
                ...
                if (canUseItem)
                {
                    ...
                    Vector2 shootDirection = Main.screenPosition + new Vector2(Main.mouseX, Main.mouseY) - playerRotation;
                    if (player.gravDir == -1f)
                    {
                        shootDirection.Y = Main.screenHeight - Main.mouseY + Main.screenPosition.Y - playerRotation.Y;
                    }
