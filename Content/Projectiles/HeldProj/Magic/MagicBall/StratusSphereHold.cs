...
using CalamityInheritance.Content.Items.Weapons.Magic.MagicBall;
using CalamityInheritance.Content.Projectiles.Magic.MagicBall;
...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Magic.MagicBall
{
    public class StratusSphereHold : CIMagicProj
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<StratusSphere>();
        ...
        public override void AI()
        {
            ...
            if (isActive && Main.myPlayer == Projectile.owner)
            {
                ...
                if (canUseItem)
                {
                    ...
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), randomSpawnOffset.X, randomSpawnOffset.Y, normalCoreVel.X, normalCoreVel.Y, ProjectileType<StratusSphereProj>(), weaponDamage2, weaponKnockback2, Projectile.owner, 0f, 0f);
                }
                else
                {
                    Projectile.Kill();
