...
using CalamityInheritance.Content.Items.Weapons.DraedonsArsenal.Magic;
...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Draedons
{
    public class GatlingLaserHeldProj : BaseHeldProj, ILocalizedModType
    {
        ...
        public override void ExAI()
        {
            ...
            {
                SoundEngine.PlaySound(CISounds.GatlingLaserFireStart, Projectile.Center);
            }
            ...
            if (Projectile.frameCounter >= 13)
            {
                ...
                if (Projectile.soundDelay <= 0)
                {
                    ...
                    gatlingLaserLoopID = SoundEngine.PlaySound(CISounds.GatlingLaserFireLoop, Projectile.Center);
                }
                if (UseDelay <= 0)
                {
        ...
        public override void OnKill(int timeLeft)
        {
            if (SoundEngine.TryGetActiveSound(gatlingLaserLoopID, out var ShootingSound) && ShootingSound.IsPlaying)
                ShootingSound.Stop();
            SoundEngine.PlaySound(CISounds.GatlingLaserFireEnd, Projectile.Center);
        }
