...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Melee.UltraGreatSword
{
    public class IridescentExcaliburSlash : ModProjectile
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<IridescentExcalibur>();
        ...
        public override void AI()
        {
            ...
            {
                Dust dust2 = Dust.NewDustPerfect(Projectile.Center + num8.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), DustID.FireworksRGB, vector3 * 1f, 100, rainbow, 0.4f);
                dust2.fadeIn = 0.4f + Main.rand.NextFloat() * 0.15f;
                dust2.noGravity = true;
            }

            if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity)
                Dust.NewDustPerfect(vector2, DustID.TintableDustLighted, vector3 * 1f, 100, rainbow * Projectile.Opacity, 1.2f * Projectile.Opacity);
            ...
            {
                Dust dust4 = Dust.NewDustPerfect(Projectile.Center + num10.ToRotationVector2() * (Main.rand.NextFloat() * 80f * Projectile.scale + 20f * Projectile.scale), DustID.FireworksRGB, vector6 * 1f, 100, rainbow, 0.4f);
                dust4.fadeIn = 0.4f + Main.rand.NextFloat() * 0.15f;
                dust4.noGravity = true;
            }

            if (Main.rand.NextFloat() * 1.5f < Projectile.Opacity)
                Dust.NewDustPerfect(vector5, DustID.TintableDustLighted, vector6 * 1f, 100, rainbow * Projectile.Opacity, 1.2f * Projectile.Opacity);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Vector2 drawpos2 = vector + (Projectile.rotation + Utils.Remap(num2, 0f, 0.6f, 0f, 0.95504415f) * Projectile.ai[0]).ToRotationVector2() * (asset.Width * 0.5f - 4f) * num;
        ...
        {
            int heal = Main.rand.Next(5, 12);

            if (Main.player[Main.myPlayer].lifeSteal <= 0f || heal <= 0 || target.lifeMax <= 5)
                return;

            Projectile.Owner().SpawnLifeStealProj(target, Projectile.GetSource_FromThis(), ProjectileType<StandardHealProj>(), target.Center, Vector2.Zero, heal);
        }
    }
}
