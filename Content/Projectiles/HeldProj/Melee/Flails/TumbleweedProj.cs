...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Melee.Flails
{
    public class TumbleweedProj : CIMeleeProj
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<TumbleweedLegacy>();
        ...
        public override void AI()
        {
            ...
            Main.player[Projectile.owner].itemRotation = (projDirection * -1f * Projectile.direction).ToRotation();
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texture2D2 = Request<Texture2D>(Texture + "Chain").Value;
            Vector2 projCenter = Projectile.Center;
            Rectangle? sourceRectangle = null;
            Vector2 origin = new Vector2(texture2D2.Width * 0.5f, texture2D2.Height * 0.5f);
            float projHeight = texture2D2.Height;
            Vector2 actualCenter = mountedCenter - projCenter;
            float projRotation = (float)Math.Atan2(actualCenter.Y, actualCenter.X) - MathHelper.PiOver2;
            bool isActive = true;
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                if (Main.rand.NextBool())
                {
                    ...
                    Main.dust[tumbleDust].fadeIn = 1f + Main.rand.Next(10) * 0.1f;
                }
            }
            for (int j = 0; j < 30; j++)
            {
            ...
            {
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center.X, Projectile.Center.Y, Projectile.velocity.X, Projectile.velocity.Y, ProjectileType<TumbleweedRollProj>(), Projectile.damage, Projectile.knockBack, Main.myPlayer, 0f, 0f);
            }
            Projectile.Kill();
        }
    }
