...
{
    public class TerraShivProj : CIBaseShortswordProjectile
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<TerraShiv>();
        public override string Texture => GetInstance<TerraShiv>().Texture;
        ...
        {
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 4f, ProjectileType<ShortTerraBeam>(), Projectile.damage * 1, Projectile.knockBack, Projectile.owner, 0f, 0f);
        };
        public override void ExtraBehavior()
        {
            if (Main.rand.NextBool(3))
            {
                int Terra = Dust.NewDust(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, DustID.Terra, Projectile.direction * 2, 0f, 15, default, 1.3f);
            }
        }
