...
{
    public class TrueNightsStabberProj : CIBaseShortswordProjectile
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<TrueNightsStabber>();
        public override string Texture => GetInstance<TrueNightsStabber>().Texture;
        ...
        {
            Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, Projectile.velocity * 4f, ProjectileType<ShortNightBeam>(), Projectile.damage * 1, Projectile.knockBack, Projectile.owner, 0f, 0f);
        };
        public override void ExtraBehavior()
        {
            if (Main.rand.NextBool(3))
            {
                int VilePowder = Dust.NewDust(Projectile.position + Projectile.velocity, Projectile.width, Projectile.height, DustID.VilePowder, Projectile.direction * 2, 0f, 15, default, 1.3f);
            }
        }
