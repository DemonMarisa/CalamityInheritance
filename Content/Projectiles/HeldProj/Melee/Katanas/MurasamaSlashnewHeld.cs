...
using CalamityInheritance.Common.CalamityModCross.ILChange;
using CalamityInheritance.Content.Items.Weapons.Melee.Katanas;
...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Melee.Katanas
{
    public class MurasamaSlashnewHeld : ModProjectile
    {
        public override LocalizedText DisplayName => LAPUtilities.GetItemName<MurasamaNeweffect>();
        private Player Owner => Main.player[Projectile.owner];
        public int time = 0;
        public override void SetStaticDefaults()
        {
            Main.projFrames[Projectile.type] = 14;
            ChangePierceResistNPC.NoPierceResist.Add(Type);
        }
        ...
        public override void AI()
        {
            ...
            if (Slash2)
            {
                SoundEngine.PlaySound(CISounds.MurasamaSwing with { Pitch = -0.1f }, Projectile.Center);
                Slashing = true;
                Projectile.numHits = 0;
            }
            else if (Slash3)
            {
                SoundEngine.PlaySound(CISounds.MurasamaBigSwing with { Pitch = 0f }, Projectile.Center);
                Slashing = true;
                Projectile.numHits = 0;
            }
            else if (Slash1)
            {
                SoundEngine.PlaySound(CISounds.MurasamaSwing with { Pitch = -0.05f }, Projectile.Center);
                Slashing = true;
                Projectile.numHits = 0;
            }
            else
        ...
        {
            if (target.Organic())
                SoundEngine.PlaySound(CISounds.MurasamaOrganicHit with { Pitch = Slash2 ? -0.1f : Slash3 ? 0.1f : Slash1 ? -0.15f : 0 }, Projectile.Center);
            else
                SoundEngine.PlaySound(CISounds.MurasamaInorganicHit with { Pitch = Slash2 ? -0.1f : Slash3 ? 0.1f : Slash1 ? -0.15f : 0 }, Projectile.Center);
