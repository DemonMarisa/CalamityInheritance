...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Melee.GreatSwords.AOTCNew
{
    public class AOTCNewParry : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            for (int k = 0; k < Main.maxProjectiles; k++)
            {
                   ...
                   Collision.CheckAABBvLineCollision(proj.Hitbox.TopLeft(), proj.Hitbox.Size(), Projectile.Owner().Center, beamEndPos, 128f, ref _))
                {
                    if (!HasParry)
                    {
                        SoundEngine.PlaySound(CISounds.ScissorGuillotineSnapSound with { Volume = CISounds.ScissorGuillotineSnapSound.Volume * 1.3f }, Projectile.Center);
                        Projectile.Owner().CI().CurAOTCCharge = CIWeaponsBlance.MaxAOTCCharge;
                        ...
                        for (int i = 0; i < 5; i++) //Don't loose your way
                        {
                            ...
                            FlareShine shine = new FlareShine(Projectile.Owner().Center + particleDispalce, particleDispalce * 0.01f, Color.White, Color.Red, 0f, new Vector2(0.6f, 1f) * particleScale, new Vector2(1.5f, 2.7f) * particleScale, 20 + Main.rand.Next(6), bloomScale: 3f, spawnDelay: Main.rand.Next(7) * 2);
                            shine.Spawn();
                        }
                        HasParry = true;
                    }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            GenericSparkle spark = new GenericSparkle(particleOrigin, Vector2.Zero, Color.White, Color.HotPink, 1.2f, 35, 0.1f, 2);
            spark.Spawn();
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D Left = CIItemTexture.SunderingScissorsLeft.Value;
            Texture2D Right = CIItemTexture.SunderingScissorsRight.Value;
