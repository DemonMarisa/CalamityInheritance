...
using CalamityInheritance.Content.VFXs;
...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Melee.GreatSwords.AOTCNew
{
    public class AOTCNewThrow : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            {
                AOTCNewStarLine.Spawn(60, Projectile.owner, Projectile.whoAmI);
                spawnFirst = true;
                ThrowTime = 0;
            }
            ...
            if (Main.rand.NextBool())
            {
                ...
                CritSpark glitter = new CritSpark(Projectile.Center + distance, Projectile.Owner().velocity + angularVelocity, Color.White, glitterColor, 1f + 1 * (distance.Length() / maxDistance), 10, 0.05f, 3f);
                glitter.Spawn();
            }
            if (Main.rand.NextBool())
            {
                ...
                for (float i = 0.5f; i <= 1; i += 0.5f)
                {
                    ...
                    HeavySmokeParticle smoke = new HeavySmokeParticle(smokepos, smokespeed, Color.Lerp(Color.DodgerBlue, Color.MediumVioletRed, i), 10 + Main.rand.Next(5), scaleFactor * Main.rand.NextFloat(2.8f, 3.1f), opacity + Main.rand.NextFloat(0f, 0.2f), 0f, false, 0, true);
                    smoke.Spawn();

                    if (Main.rand.NextBool(3))
                    {
                        HeavySmokeParticle smokeGlow = new HeavySmokeParticle(smokepos, smokespeed, Main.rand.NextBool(5) ? Color.Gold : Color.Chocolate, 7, scaleFactor * Main.rand.NextFloat(2f, 2.4f), opacity * 2.5f, 0f, true, 0.004f, true);
                        smokeGlow.Spawn();
                    }
                }
            }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D sword = CIItemTexture.SunderingScissorsLeft.Value;
            ...
            if (ChargeAttack)
            {
                Texture2D sword2 = CIItemTexture.SunderingScissorsRight.Value;
