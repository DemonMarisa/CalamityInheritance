    ...
    public class AOTCNewBlast : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            if (!AddEndParticle && AniHelper.HasFinish[AniState.Begin])
            {
                ...
                SoundEngine.PlaySound(CISounds.SwiftSlice with { Pitch = 0f, MaxInstances = 2 }, Projectile.Center);
                ScreenShakeSystem.AddScreenShakes(Projectile.Center, 7.5f, 15, 0);
                for (int i = 0; i < 60; i++)
                {
                ...
                for (int i = 0; i < Max; i++)
                {
                    ...
                    CritSpark spark = new CritSpark(stitchCenter, Vector2.Zero, Color.White, Color.Cyan, 3f, 8, 0.1f, 3);
                    spark.Spawn();
                }
                ...
                if (LAPUtilities.IsLocalPlayer(Projectile.owner))
                {
                    ...
                    for (int s = 1; s <= starAmt; s++)
                    {
                        ...
                        Projectile blast = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), starPosition, Main.rand.NextVector2CircularEdge(12, 12), ProjectileType<EonBoltNew>(), (int)(0.2f * Projectile.damage), 0f, Projectile.owner, 0.55f, MathHelper.Pi * 0.07f);
                    }
                }
