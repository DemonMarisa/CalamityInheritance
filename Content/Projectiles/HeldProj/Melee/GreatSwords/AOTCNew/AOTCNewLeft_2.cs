...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Melee.GreatSwords.AOTCNew
{
    public class AOTCNewLeft_2 : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.LAP().FirstFrame)
            {
                ...
                if (ChargeAttack)
                    sound = CISounds.LouderPhantomPhoenix;
            ...
            if (firetime > 4 && firecount < 6 && Projectile.Owner().whoAmI == Main.myPlayer/* && (Projectile.timeLeft - 1) % Math.Ceiling(MaxSwingTime / 6f) == 0f*/)
            {
                ...
                Projectile.NewProjectileDirect(source, Projectile.Owner().Center + adjustedBlastRotation.ToRotationVector2() * 10f, adjustedBlastRotation.ToRotationVector2() * 8, ProjectileType<EonBoltNew>(), (int)(0.7f / 6f * Projectile.damage), 0f, Projectile.Owner().whoAmI, 0.55f, MathHelper.Pi * 0.05f);
            }
        ...
        public void SpawnPartic()
        {
            ...
            if (Main.rand.NextBool())
            {
                ...
                Vector2 angularVelocity = distance.RotatedBy(MathHelper.PiOver2 * Projectile.Owner().direction).SafeNormalize(Vector2.Zero) * 2 * (1f + distance.Length() / 15f);
                CritSpark glitter = new CritSpark(Projectile.Owner().Center + distance, Projectile.Owner().velocity + angularVelocity, Main.rand.NextBool(3) ? Color.Turquoise : Color.Coral, currentColor, 1f + 1 * (distance.Length() / maxDistance), 10, 0.05f, 3f);
                glitter.Spawn();
            }
            ...
            for (float i = 0f; i <= 1; i += 0.5f)
            {
                ...
                if (Main.rand.NextBool())
                {
                    HeavySmokeParticle smoke = new HeavySmokeParticle(smokepos, smokespeed, Color.Lerp(Color.DodgerBlue, Color.MediumVioletRed, i), 6 + Main.rand.Next(5), scaleFactor * Main.rand.NextFloat(2.8f, 3.1f), Opacity + Main.rand.NextFloat(0f, 0.2f), 0f, false, 0, true);
                    smoke.Spawn();
                }
                if (Main.rand.NextBool(4))
                {
                    HeavySmokeParticle smokeGlow = new HeavySmokeParticle(smokepos, smokespeed, Main.rand.NextBool(5) ? Color.Gold : Color.Chocolate, 5, scaleFactor * Main.rand.NextFloat(2f, 2.4f), Opacity * 2.5f, 0f, true, 0.004f, true);
                    smokeGlow.Spawn();
                }
            }
        }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D smear = CIExtraTexture.TrientCircularSmear.Value;
            ...
            {
                Texture2D texture = CIItemTexture.SunderingScissorsLeft.Value;
                Vector2 DrawOffset = new Vector2(5, 0).RotatedBy(Projectile.rotation);
                float rot = Projectile.direction == -1 ? Projectile.rotation + MathHelper.PiOver4 : Projectile.rotation - MathHelper.PiOver4 + MathHelper.Pi;
                Vector2 orig = Projectile.direction == -1 ? new Vector2(0, texture.Height) : new Vector2(texture.Width, texture.Height);
                SpriteEffects effect = Projectile.direction == -1 ? SpriteEffects.None : SpriteEffects.FlipHorizontally;
