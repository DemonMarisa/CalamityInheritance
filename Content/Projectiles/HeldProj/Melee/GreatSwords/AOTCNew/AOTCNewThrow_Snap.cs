...
namespace CalamityInheritance.Content.Projectiles.HeldProj.Melee.GreatSwords.AOTCNew
{
    public class AOTCNewThrow_Snap : ModProjectile, ILocalizedModType
    {
        ...
        public override void AI()
        {
            ...
            {
                ScreenShakeSystem.AddScreenShakes(Projectile.Center, 3f, 15, 0);
                SoundEngine.PlaySound(SoundID.Item84, Projectile.Center);
                Vector2 sliceDirection = ToPlayerDistance.SafeNormalize(Vector2.UnitX) * 40;
                LineVFX SliceLine = new LineVFX(Projectile.Center - sliceDirection, sliceDirection * 2f, 0.2f, Color.Orange * 0.7f, expansion: 250f)
                {
                    Lifetime = 10
                };
                SliceLine.Spawn();
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Projectile.LAP().OnceHitEffect)
            {
                ...
                Projectile.Owner().CI().CurAOTCCharge += 2;
            }
        }
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D swordR = CIItemTexture.SunderingScissorsRight.Value;
