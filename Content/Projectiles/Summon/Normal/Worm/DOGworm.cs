...
using CalamityInheritance.Content.Projectiles.Summon.Proj.Worm.DOGWorm;
...
namespace CalamityInheritance.Content.Projectiles.Summon.Normal.Worm
{
    ...
    public class DOGworm : CISummonProj
    {
        public override string Texture => CIProjectiles_Summon.DOGworm_Head.Path;
        ...
        public override void AI()
        {
            ...
            NPC target = Projectile.Center.MinionHoming(1500, Owner);
        ...
        public void PlayerFollowMovement(Player owner)
        {
            ...
            else if (Projectile.Center.Distance(Owner.Center) > 300)
            {
        ...
        {
            Texture2D tail = CIProjectiles_Summon.DOGworm_Tail.Value;
            Vector2 TaildrawPos = Segments[^1].Pos - Main.screenPosition;
            Vector2 Tailorig = tail.Size() / 2;
            Main.spriteBatch.Draw(tail, TaildrawPos, null, lightColor * Projectile.Opacity, Segments[^1].Rot + MathHelper.PiOver2, Tailorig, 1f, 0, 0);
            for (int i = 1; i < Segments.Count - 1; i++)
            {
                Texture2D body = CIProjectiles_Summon.DOGworm_Body.Value;
                Vector2 bodydrawPos = Segments[i].Pos - Main.screenPosition;
                Vector2 bodyorig = body.Size() / 2;
                Main.spriteBatch.Draw(body, bodydrawPos, null, lightColor * Projectile.Opacity, Segments[i].Rot + MathHelper.PiOver2, bodyorig, 1f, 0, 0);
            }
            Texture2D texture = TextureAssets.Projectile[Type].Value;
            Vector2 drawPos = Segments[0].Pos - Main.screenPosition;
            Vector2 orig = new Vector2(texture.Width / 2, texture.Height - 11);
            Main.spriteBatch.Draw(texture, drawPos, null, lightColor * Projectile.Opacity, Segments[0].Rot + MathHelper.PiOver2, orig, 1f, 0, 0);
            return false;
        }
    }
}
