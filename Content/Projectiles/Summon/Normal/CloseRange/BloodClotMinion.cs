...
namespace CalamityInheritance.Content.Projectiles.Summon.Normal.CloseRange
{
    internal class BloodClotMinion : CISummonProj
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] == 0f)
            {
                ...
                for (int num227 = 0; num227 < num226; num227++)
                {
                    ...
                    int num228 = Dust.NewDust(vector6 + vector7, 0, 0, DustID.Blood, vector7.X * 1.5f, vector7.Y * 1.5f, 100, default, 1.4f);
            ...
            int num1262 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.Blood, 0f, 0f, 0, default, 1f);
            ...
            player.AddBuff(BuffType<BloodClotBuff>(), 3600);
            if (flag64)
            {
                if (player.dead)
                {
            ...
            if (Math.Abs(Projectile.Center.X - player.Center.X) + Math.Abs(Projectile.Center.Y - player.Center.Y) > num538)
            {
                Projectile.ai[0] = 1f;
            }
            if (Projectile.ai[0] == 0f)
            {
                if (player.HasMinionAttackTargetNPC)
                {
                    ...
                    {
                        float num539 = npc.position.X + npc.width / 2;
                        float num540 = npc.position.Y + npc.height / 2;
                        float num541 = Math.Abs(Projectile.position.X + Projectile.width / 2 - num539) + Math.Abs(Projectile.position.Y + Projectile.height / 2 - num540);
                ...
                if (!flag19)
                {
                    for (int num542 = 0; num542 < Main.maxNPCs; num542++)
                    {
                        ...
                        {
                            float num543 = Main.npc[num542].position.X + Main.npc[num542].width / 2;
                            float num544 = Main.npc[num542].position.Y + Main.npc[num542].height / 2;
                            float num545 = Math.Abs(Projectile.position.X + Projectile.width / 2 - num543) + Math.Abs(Projectile.position.Y + Projectile.height / 2 - num544);
            ...
            if (!flag19)
            {
                ...
                Vector2 vector42 = new Vector2(Projectile.position.X + Projectile.width * 0.5f, Projectile.position.Y + Projectile.height * 0.5f);
            ...
            else
            {
                ...
                {
                    float num550 = 8f; //12
                    Vector2 vector43 = new Vector2(Projectile.position.X + Projectile.width * 0.5f, Projectile.position.Y + Projectile.height * 0.5f);
        ...
        {
            target.AddBuff(BuffType<CIBurningBlood>(), 120);
            Projectile.ai[1] = -1f;
