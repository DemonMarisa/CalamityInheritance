...
namespace CalamityInheritance.Content.Projectiles.Summon.Normal.Fighter
{
    public class MidnightSunUFOold : CISummonProj
    {
        ...
        public override void AI()
        {
            ...
            if (potentialTarget != null)
            {
                if (Projectile.ai[0]++ % 360 < 180)
                {
                    ...
                    {
                        // 伤害再/2是因为生成后莫名其妙伤害翻倍
                        Projectile o = Projectile.NewProjectileDirect(Projectile.GetSource_FromThis(), Projectile.Bottom, Projectile.DirectionTo(potentialTarget.Center).RotatedByRandom(0.15f) * 25f, ProjectileType<MidnightSunLaserold>(), Projectile.damage, Projectile.knockBack, Projectile.owner);
                        o.originalDamage = Projectile.originalDamage;
                    }
                ...
                else
                {
                    ...
                    if (Projectile.ai[0] % 180 < 180 - framesUsedSpinning)
                    {
                        ...
                        Vector2 spawnPosition = Projectile.Center + Main.rand.NextVector2Unit().RotatedBy(Projectile.rotation) * new Vector2(13f, 6f) / 2f;
