...
using CalamityInheritance.Content.Projectiles.Summon.Proj.Limits;
...
namespace CalamityInheritance.Content.Projectiles.Summon.Normal.Limits
{
    public class SarosAuraLegacy : CISummonProj
    {
        ...
        public void AttackTarget(NPC target, int radiantOrbAppearRate, int radiantOrbDamage, int radiantOrbOriginalDamage)
        {
            if (GeneralTimer % 35f == 34f)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Vector2 fireVelocity = LAPUtilities.GetVector2(Projectile.Center, target.Center).RotatedBy(angle) * 15f;
            ...
            if (GeneralTimer % radiantOrbAppearRate == radiantOrbAppearRate - 1)
            {
                ...
                Vector2 bootlegRadianceOrbVelocity = LAPUtilities.GetVector2(Projectile.Center, target.Center) * 2f;
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Vector2 fireVelocity = LAPUtilities.GetVector2(Projectile.Center, target.Center).RotatedBy(angle) * 19f;
                    int p = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, fireVelocity, ProjectileType<SarosSunfireLegacy>(), radiantOrbDamage / 2, Projectile.knockBack, Projectile.owner);
                    if (Main.projectile.IndexInRange(p))
                        Main.projectile[p].originalDamage = radiantOrbOriginalDamage / 2;
                }
