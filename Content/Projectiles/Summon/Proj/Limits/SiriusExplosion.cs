...
namespace CalamityInheritance.Content.Projectiles.Summon.Proj.Limits
{
    public class SiriusExplosion : CISummonProj
    {
        ...
        public override void AI()
        {
            ...
            if (activeTimer < 0 || activeTimer >= 1000 || (!Main.projectile[activeTimer].active))
            {
                Projectile.ai[1] = -1f;
            }
            else
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) => target.AddBuff(ModContent.BuffType<CINightwither>(), 180);
    }
}
